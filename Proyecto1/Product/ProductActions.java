...
public class ProductActions {
    ...
    public static void openEditProductWindow(JTable table, DefaultTableModel model) {
        if (ProductDAO.getInstance().products.isEmpty()) {
            JOptionPane.showMessageDialog(null, "No hay productos disponibles para editar.", "Información",
    ...
    public static void openDeleteProductWindow(JTable table, DefaultTableModel model) {
        if (ProductDAO.getInstance().products.isEmpty()) {
            JOptionPane.showMessageDialog(null, "No hay productos disponibles para eliminar.", "Información",
                    JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        deleteButton.addActionListener(e -> {
            ...
            if (selectedItem != null) {
                int confirm = JOptionPane.showConfirmDialog(frame, "¿Está seguro de eliminar el producto seleccionado?",
                        "Confirmar Eliminación", JOptionPane.YES_NO_OPTION);

                if (confirm == JOptionPane.YES_OPTION) {
                    int productId = Integer.parseInt(selectedItem.split(" - ")[0]);

                    boolean removed = ProductDAO.getInstance().products
                    ...
                    if (removed) {
                        ...
                        JOptionPane.showMessageDialog(frame, "Producto no encontrado.", "Error",
                                JOptionPane.ERROR_MESSAGE);
                    }
                }
            } else {
                JOptionPane.showMessageDialog(frame, "No se ha seleccionado ningún producto.", "Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        });
    ...
    public static void generateTopSoldReport() {
        try {
            if (ProductDAO.getInstance().products.isEmpty()) {
                JOptionPane.showMessageDialog(null, "No hay productos disponibles para generar el reporte.", "Información",
                        JOptionPane.INFORMATION_MESSAGE);
                return;
            }
